...
public class OwnerController {
	...
	// 회원 리스트
	@GetMapping("/list")
	public ResponseEntity<Object> ownerList(Pageable pageable) {
		Page<OwnerListResDto> owners = ownerService.ownerList(pageable);
		return new ResponseEntity<>(new CommonResDto(HttpStatus.OK, "회원 리스트", owners.toList()), HttpStatus.OK);
	}

	// 회원 정보(마이페이지) 조회하는 api필요 -> 토큰으로 처리
	// public ResponseEntity<?> ownerDetail(){
	//
	// }
}
